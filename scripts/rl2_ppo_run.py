...
from algos.rl2_ppo.learning import train_rl2_ppo_meta_reset, meta_evaluate_policy
...
class DictToTensorWrapper(gym.Wrapper):
    ...
    def meta_reset(self, **kwargs):
        ...
        """
        # if hasattr(self.env, "meta_reset"):
        obs, info = self.env.meta_reset(**kwargs)
        return self._convert_obs(obs), info
        # else:
        #     # fallback
        #     print("Origin Env dont have meta_reset func")
...
def main(config):
    # 初始化环境
    TASK_NAME = "Lift-Cube-UR5e-Allegro-MetaRL"
    ...
    # 启动训练
    train_rl2_ppo_meta_reset(config, train_envs, test_envs, writer)
